...
import { useTaskContext } from '../TaskContext'; // Импорт из src/
...
const ParticipantsBoard = ({ projectId }) => {
  ...
  const { showParticipantModal, setShowParticipantModal, refreshParticipants } = useTaskContext(); // Используем контекст
  ...
  const handleAddParticipant = () => {
    console.log('Opening AddParticipantsModal, projectId:', projectId); // Отладка
              ...
              <Card.Body className="d-flex flex-column align-items-center text-center">
                <div
                  style={{
                    ...
                    height: '50px',
                    borderRadius: '50%',
                    backgroundImage: participant.user?.avatar
                      ? `url(http://localhost:8000/${participant.user.avatar})`
                ...
                >
                  {!participant.user?.avatar &&
                    (participant.user?.username?.[0]?.toUpperCase() || `У${index + 1}`)}
                </div>
