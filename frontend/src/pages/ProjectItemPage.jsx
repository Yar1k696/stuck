...
import ExecutorsBoard from '../components/ExecutorsBoard';
...
const ProjectItemPage = () => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await fetch(`/api/projects/${pk}/tasks/`, {
        method: 'GET',
        credentials: 'include',
      });
      if (response.ok) {
        const data = await response.json();
        setTasks(data);
        const uniqueExecutors = [...new Set(data.map(task => task.assigned_to).filter(user => user))];
        setExecutors(uniqueExecutors.length > 0 ? uniqueExecutors : project?.executors || []);
      } else {
  ...
  }, [pk, refreshTasks, setRefreshTasks]);
  ...
  return (
    <Container className="mt-4">
            ...
            <Card.Body>
              <Row>
                ...
                <Col md={8}>
                  <Row>
                    <Col md={12}>
                      <Card.Title>Учасники проекту:</Card.Title>
                      <ParticipantsBoard projectId={projectId} />
                    </Col>
                    <Col md={12}>
                      <Card.Title>Виконавці проекту:</Card.Title>
                      <ExecutorsBoard project={project} tasks={tasks} />
                    </Col>
                  </Row>
                </Col>
              </Row>
      ...
      <div className="mt-5">
        ...
        <TasksBoard projectId={projectId} refreshTasks={refreshTasks} setTaskAction={setTaskAction} />
      </div>
    </Container>
  );
};
